...
**Микрофон внутри веб-версии:** после сборки исходников (`npm run build:plugin`)
запусти `opencode web --port 4096` и в другом терминале `node dist/cli.js voice-web`.
Открой http://127.0.0.1:4097 — микрофон появится рядом с отправкой, открытая сессия
определяется автоматически. Распознанный текст добавляется в черновик без отправки.
Нужны установленные локальный Whisper и модель из voice-overlay.
При переключении сессии во время записи результат сохраняется в уведомлении;
для вставки вернись в исходную сессию и нажми микрофон.
...
Перед публикацией `prepublishOnly` запускает полный check, а `prepack` пересобирает plugin и dashboard. Публикация автоматическая: сделайте commit и Push origin в `main` через GitHub Desktop. Workflow `Publish to npm` выбирает следующую patch-версию по Git-тегам и npm, создаёт тег, собирает и публикует voice-пакеты, затем основной пакет. Версию в локальном package.json менять не требуется; повышенную вручную major/minor-версию workflow учитывает. При ошибке используйте Re-run all jobs: версия сохраняется, уже опубликованные пакеты пропускаются. Для восстановления старого релиза можно запустить workflow вручную с существующим тегом. Публикации выполняются последовательно; при нескольких быстрых push GitHub может заменить ожидающий запуск более свежим. Первый push с этим workflow включает автоматизацию.
