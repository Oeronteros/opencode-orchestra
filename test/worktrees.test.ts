...
test("creates isolated deterministic worktree",async()=>{const git=new FakeGit([{exitCode:0}]); const result=await createEditorWorktree(git,"repo","task 1","api","base","root"); assert.equal(result.branch,"orch/task-1/api"); assert.deepEqual(git.calls[0],["worktree","add","-b","orch/task-1/api",path.join("root","task-1-api"),"base"])})
test("uses a safe default when worktree root is omitted",async()=>{const git=new FakeGit([{exitCode:0}]); const result=await createEditorWorktree(git,"repo","task","api","base"); assert.equal(result.path,path.join(".orchestra","worktrees","task-api")); assert.deepEqual(git.calls[0],["worktree","add","-b","orch/task/api",path.join(".orchestra","worktrees","task-api"),"base"])})
...
test("aborts the whole cherry-pick sequence on conflict",async()=>{const git=new FakeGit([{exitCode:1,stderr:"conflict"},{exitCode:0}]); await assert.rejects(()=>integrateValidatedCommits(git,"repo",["a","b"])); assert.deepEqual(git.calls,[["cherry-pick","a","b"],["cherry-pick","--abort"]])})
