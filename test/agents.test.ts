...
test("lead can invoke Orchestra workers but workers cannot delegate", () => {
  ...
  assert.equal(agents["orch-lead"]?.permission.bash, "allow")
  assert.equal(agents["orch-lead"]?.permission["memorygraph_*"], "allow")
  assert.equal(agents["orch-lead"]?.permission["git_*"], "allow")
  ...
  assert.equal(agents["orch-repo"]?.permission["git_*"], "allow")
