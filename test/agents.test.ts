...
test("builds a selectable primary lead and hidden workers", () => {
  ...
  assert.equal(typeof agents["orch-editor"]?.permission.bash, "object")
...
test("evidence workers delegate only through the guarded dispatcher", () => {
  ...
  assert.equal((taskPermission as Record<string, string>)["orch-editor"], "deny")
  assert.equal((taskPermission as Record<string, string>)["orch-integrator"], "deny")
  ...
  assert.equal(agents["orch-lead"]?.permission["git_*"], "deny")
  assert.equal(agents["orch-lead"]?.permission.git_git_status, "allow")
  assert.equal(agents["orch-lead"]?.permission.git_git_commit, "ask")
  ...
  assert.equal(typeof agents["orch-tests"]?.permission.bash, "object")
  assert.equal((agents["orch-tests"]?.permission.bash as Record<string, string>)["*"], "ask")
