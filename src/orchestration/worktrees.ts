...
export async function createEditorWorktree(git: GitRunner, repo: string, taskId: string, nodeId: string, baseSha: string, root: string): Promise<{ path: string; branch: string }> { const safe = (value: string) => value.replace(/[^a-zA-Z0-9._-]/g, "-"); const branch = "orch/" + safe(taskId) + "/" + safe(nodeId); const path = root + "/" + safe(taskId) + "-" + safe(nodeId); const result = await git.run(["worktree", "add", "-b", branch, path, baseSha], repo); if (result.exitCode !== 0) throw new Error(result.stderr || "git worktree add failed"); return { path, branch } }
