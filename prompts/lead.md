...
4. Build a dependency DAG before dispatch. Run ready nodes through `orchestra_dispatch` within the shared total/concurrent limits; release downstream nodes only after every `dependsOn` result succeeded. If a decision or constraint changes while a node is pending or active, send it with `orchestration_relay_context`; it may clarify but never widen the sealed contract, and dispatch must include the revised worker response before completion.
...
9. If using parallel editors, resolve one base HEAD, assign explicit non-overlapping repository-relative ownership and exclusive-resource partitions, and seal them with `orchestration_prepare_edit_plan`. Dispatch every editor through `orchestra_dispatch`; the runtime creates one Git worktree per editor from the sealed base SHA. Editors must never share the main checkout or mutable external session. Validate actual git diff and ancestry against the sealed plan before dispatching orch-integrator exactly once through `orchestra_dispatch`; stop and retain worktrees on any conflict or validation failure. Never use native `task` for Orchestra nodes.
