...
Whisper из установки voice-overlay с выбранной моделью `base` или `small`; запись и перевод
...
кнопкой OpenCode. Через ⚙ рядом с микрофоном можно выбрать устройство, Whisper-модель
и действие «Вставить текст» (по умолчанию) либо «Вставить и отправить».
Встроенная кнопка всегда использует текущий composer, независимо от сохранённых
ручных назначений. Выбора TUI/Web и сессии здесь нет.
Если во время записи или распознавания перейти в другую сессию,
...
Восстановление результата всегда вставляет текст без автоматической отправки.
При недоступной Send-кнопке текст остаётся в composer; отложенная отправка не ставится в очередь.

### Remote Voice / fallback

`http://127.0.0.1:4097/voice` — отдельная страница с выбором сессии и обновлением списка.
Она переиспользует тот же browser recorder и STT endpoint. При стандартной настройке
результат появляется в редактируемом превью; кнопка «Отправить в сессию» вызывает
`prompt_async` выбранной сессии. Режим «Вставить и отправить» включает автоподачу
в сессию, выбранную при начале записи. Настройки и последний выбор сохраняются.
В отличие от встроенной кнопки, эта страница не имеет доступа к composer другой вкладки.
Proxy сохраняет loopback binding и проверки origin: доступ с другого устройства
требует защищённого туннеля с локальным адресом на клиенте. Открытого LAN-listener
или новой схемы аутентификации эта доработка не добавляет.
...
**Открытая вкладка (Auto)**: запустите `opencode-orchestra voice-web --upstream http://127.0.0.1:4096`
и откройте нужную сессию через **http://127.0.0.1:4097**. В настройках оверлея выберите
«Открытая вкладка браузера»; порт вкладки по умолчанию — 4097. Нажмите в поле ввода
нужной вкладки, затем на микрофон оверлея. После распознавания текст добавится в её
редактор; **отправку нажимаете вы**, даже если раньше была включена автоотправка.
Это работает и с черновиками новых сессий, сохраняет уже введённый текст, модель и агента.

Получатель фиксируется при начале записи. Если вкладка закрыта, скрыта или перешла
в другую сессию, текст остаётся в оверлее. Вернитесь в исходную вкладку и нажмите
«Повторить вставку в исходную вкладку». До отправки/удаления этого текста новая
запись в Auto не начинается. При потере подтверждения проверьте поле ввода перед
повтором, чтобы вручную не продублировать текст. Вкладки на прямом порту OpenCode
(4096) не имеют этой связи — откройте тот же путь через voice-web (4097).
Подключённая видимая вкладка определяется по последнему фокусу; переход фокуса
в сам оверлей сохраняет получателя. Обмен остаётся локальным, Windows и Linux используют
один протокол. Ручные режимы TUI и Web-сессии по-прежнему доступны.

Предусловие: `opencode web --port 4096` (та же Basic-auth через `OPENCODE_SERVER_PASSWORD`). В разделе «Дополнительно» настроек оверлея переключи назначение на «Web-сессия» и выбери сессию из списка. Оверлей запрашивает до 1000 последних корневых сессий всех проектов через `GET /experimental/session?roots=true&limit=1000`, скрывает архивные и показывает каталог рядом с названием. На старых серверах с ответом 404 используется `/session`, ограниченный проектом сервера. Список обновляется каждые 15 секунд и при возвращении фокуса в главное окно; в настройках — при изменении подключения или вручную. Перед отправкой оверлей получает каталог выбранной сессии и передаёт его в `prompt_async`, чтобы запрос выполнялся в правильном проекте. Назначение по умолчанию — Auto, связанное с полем ввода открытой вкладки через voice-web. Ручные назначения из старых настроек сохраняются.

По умолчанию автоотправки нет. После транскрипции ручной Web-режим показывает превью
и кнопку «Отправить в сессию». Настройка «Вставить и отправить» включает автоподачу;
для TUI после успешной вставки используется `/tui/submit-prompt`.
TUI API адресует активный input сервера, а не конкретный session ID: при переключении
сессии непосредственно в терминале во время STT контекст нельзя надёжно проверить.
Для прежней механики терминального ввода выберите явный режим TUI.

### Совместимость настроек

- Существующий ключ overlay `voice-overlay-settings:v1` сохранён; `target: "tui" | "web"`
  и `sessionId`, микрофон, модель и параметры подключения продолжают читаться.
- Добавлены `target: "auto"` и `postTranscriptionAction: "insert" | "insert-and-submit"`.
  Отсутствующие/некорректные поля получают безопасные defaults; legacy `destination` тоже читается.
- Browser-настройки лежат отдельно в `orchestra-voice-settings:v1`, общие для встроенной
  кнопки и `/voice` на одном origin. Tauri и браузер имеют разные хранилища и разные
  идентификаторы микрофонов; автоматической синхронизации между ними нет.

### Проверка интеграции

`npm test` включает policy/config tests и DOM/media contract tests. Для воспроизводимой
ручной проверки браузерных событий без физического микрофона и без LLM:
...
Откройте `http://127.0.0.1:4197`: проверьте Insert (Submits: 0), InsertAndSubmit
(Submits: 1), перезагрузку настроек, Rerender и переход Session A → B во время STT.
Это DOM fixture, не замена финальному E2E с настоящим OpenCode и микрофоном.
...
- Нет глобального хоткея, стриминга STT, waveform и облачного fallback.
