...
вариант для старых TUI; оно больше не предлагает назначение Web/вкладки.
Для окна нужен `opencode --port 4096`.
...
## Предусловие только для ручного окна: живой TUI с фиксированным портом
...
Настройки микрофона, модели и подключения старого окна сохраняются в
`voice-overlay-settings:v1`; при загрузке назначение принудительно становится TUI,
а действие после распознавания — вставкой без отправки.
### Проверка интеграции
...
Чеклист Web: открыть `opencode web --port 4096` и прокси на 4097, создать сессию, наговорить 5 секунд через встроенную кнопку → текст появляется в текущем черновике. Проверить смену сессии во время распознавания и сохранение невставленного результата.
