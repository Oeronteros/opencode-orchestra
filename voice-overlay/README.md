...
останавливает запись. Лимит — 120 секунд. Распознавание выполняет локальный
...
текст останется в уведомлении: вернитесь в исходную сессию и нажмите микрофон
для вставки. До этого новая запись заблокирована. Перезагрузка страницы удаляет
этот временный результат.

Другие порты: `node dist/cli.js voice-web --upstream http://127.0.0.1:4096 --port 4097`.
