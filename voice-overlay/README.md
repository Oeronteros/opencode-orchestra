...
Предусловие: `opencode web --port 4096` (та же Basic-auth через `OPENCODE_SERVER_PASSWORD`). В разделе «Дополнительно» настроек оверлея переключи назначение на «Web-сессия» и выбери сессию из списка (`GET /session`). Новое назначение по умолчанию — Auto; для desktop overlay это прежний TUI input. Ручные назначения из старых настроек сохраняются.
