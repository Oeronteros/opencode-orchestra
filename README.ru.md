...
### Переход на OpenCode 2

После обновления OpenCode повторно запустите `install`. Версия 2 загружает плагин через новый API `setup`; установщик заранее создаёт имена агентов Orchestra в конфигурации, поскольку плагин V2 может изменять существующих агентов, но не создавать их через `agent.transform`. Для OpenCode 1.18.29 и новее сохранён прежний входной модуль. Поля конфигурации V1 OpenCode 2 преобразует при загрузке, поэтому переписывать конфигурацию необязательно. См. [официальное руководство](https://opencode.ai/v2/docs/build/plugins/migrate-v1).
